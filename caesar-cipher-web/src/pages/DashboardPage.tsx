...
function EncryptPanel() {
  ...
  const [result, setResult] = useState<{ hash: string; encrypted: string } | null>(null);
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-lg shadow-slate-200/40 dark:shadow-none">
        <div className="flex items-center justify-between mb-5">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Criptografar Mensagem</h2>
          </div>
        ...
        <form onSubmit={handleEncrypt} className="space-y-4">
          <div>
            <label htmlFor="encrypt-message" className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1.5">
              Mensagem Original
            </label>
            <textarea
              id="encrypt-message"
          ...
          <div>
            <label htmlFor="encrypt-step" className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1.5">
              Passo (deslocamento)
            </label>
            <div className="relative">
              ...
              <input
                ...
                type="number"
                value={step}
                onChange={(e) => setStep(e.target.value === '' ? '' : parseInt(e.target.value, 10))}
                placeholder="Ex: 5"
                className="w-full pl-11 pr-4 py-3 bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-xl text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 shadow-sm transition-all duration-200"
              />
            </div>
            <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">Charset: a-z + 0-9 (36 posições)</p>
          </div>
          ...
          {message && typeof step === 'number' && !result && (
            <div className="p-4 bg-indigo-50 dark:bg-indigo-500/10 rounded-xl border border-indigo-200 dark:border-indigo-500/20 animate-fade-in">
              <span className="block text-xs font-medium text-indigo-700 dark:text-indigo-400 mb-1">Pré-visualização (não salva):</span>
      ...
      {result && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-lg shadow-slate-200/40 dark:shadow-none animate-slide-up">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Resultado Salvo</h3>
          </div>

          {/* Encrypted message */}
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1.5">
              <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Mensagem Cifrada</span>
              <button
          ...
          <div>
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Hash (chave privada)</span>
              </div>
              <button
...
function DecryptPanel() {
  ...
  const handleDecrypt = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await cipherService.decrypt(encryptedMessage, hash.trim());
      if (response.success && response.data) {
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-lg shadow-slate-200/40 dark:shadow-none">
        <div className="flex items-center gap-2 mb-5">
          ...
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Descriptografar Mensagem</h2>
        </div>

        <form onSubmit={handleDecrypt} className="space-y-4">
          <div>
            <label htmlFor="decrypt-message" className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1.5">
              Mensagem Cifrada
            </label>
            <textarea
              id="decrypt-message"
          ...
          <div>
            <label htmlFor="decrypt-hash" className="block text-sm font-medium text-slate-500 dark:text-slate-400 mb-1.5">
              Hash (chave privada)
            </label>
            <div className="relative">
      ...
      {result !== null && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-lg shadow-slate-200/40 dark:shadow-none animate-slide-up">
          <div className="flex items-center gap-2 mb-5">
            ...
            <h3 className="text-lg font-semibold text-slate-900 dark:text-white">Mensagem Original</h3>
          </div>

          <div className="flex items-center justify-between mb-1.5">
            <span className="text-sm font-medium text-slate-700 dark:text-slate-300">Resultado</span>
            <button
          ...
          <p className="mt-3 text-xs text-slate-500 dark:text-slate-400 flex items-center gap-1">
            ...
            O hash utilizado foi marcado como usado e não pode ser reutilizado.
          </p>
        </div>
      )}
    </div>
    ...
    <div className="space-y-6">
      <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 sm:p-8 shadow-lg shadow-slate-200/40 dark:shadow-none">
        <div className="flex items-center gap-2 mb-6">
          ...
          <h2 className="text-lg font-semibold text-slate-900 dark:text-white">Seus Hashes Gerados</h2>
        </div>
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 px-4 rounded-xl border border-dashed border-slate-300 dark:border-slate-700">
            <p className="text-slate-600 dark:text-slate-400">Nenhum hash gerado ainda.</p>
            <p className="text-xs text-slate-500 dark:text-slate-500 mt-1">Criptografe uma mensagem para começar.</p>
          </div>
        ) : (
          <div className="space-y-4">
            {history.map((item) => {
              const isExpired = new Date(item.expiresAt) < new Date();
              const status = item.used ? 'usado' : isExpired ? 'expirado' : 'disponível';

              return (
                <div 
                  key={item._id} 
                  className={`p-4 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4 transition-all ${
                    status === 'disponível' ? 'bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700' :
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${
                        status === 'disponível' ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-400' :
                        status === 'usado' ? 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400' :
                  ...
                  <div className="flex items-center gap-4 text-xs text-slate-600 dark:text-slate-400 shrink-0 border-t border-slate-200 dark:border-slate-700 pt-3 sm:border-0 sm:pt-0">
                    <div className="flex items-center gap-1" title="Passo (deslocamento)">
                    ...
                    <div className="flex items-center gap-1" title="Expira em">
                      ...
                      <span>
                        {item.expiresAt && isValid(parseISO(item.expiresAt))
                          ? format(parseISO(item.expiresAt), "dd/MM 'às' HH:mm", { locale: ptBR })
                          : 'Data indisponível'}
                      </span>
                    </div>
                  </div>
